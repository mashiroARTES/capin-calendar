...
function renderContent() {
  ...
  el.style.display  = State.viewMode === 'quick' ? 'flex'   : 'block';
...
function renderQuickView() {
  ...
  const cellsHtml = cells.map(function(d) {
    if (!d) return '<div style="min-width:0"></div>';
    ...
    const cellSz = isPC ? '28px' : '20px';
    const fSz    = isPC ? '13px' : '12px';
    const dotSz  = isPC ? '5px'  : '3px';
    return '<div onclick="selectQuickDate(\'' + ds + '\')" style="min-width:0;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;-webkit-tap-highlight-color:transparent;padding:' + (isPC?'1px':'0') + ' 0">'
  ...
  const miniCal =
    ...
    + (isPC
        ...
        : 'background:#fff;border-bottom:2px solid #e5e7eb;flex-shrink:0;overflow:visible;')
    + '">'
    + '<div style="padding:' + (isPC?'6px':'2px') + ' 8px 0;display:flex;align-items:center;justify-content:space-between">'
    + '<button onclick="navPrev()" style="background:none;border:none;cursor:pointer;padding:' + (isPC?'4px 10px':'1px 8px') + ';font-size:' + (isPC?'16':'15') + 'px;color:#6b7280">&#8249;</button>'
    + '<span style="font-size:' + (isPC?'14':'12') + 'px;font-weight:700;color:#374151">'+y+'年'+m+'月</span>'
    + '<button onclick="navNext()" style="background:none;border:none;cursor:pointer;padding:' + (isPC?'4px 10px':'1px 8px') + ';font-size:' + (isPC?'16':'15') + 'px;color:#6b7280">&#8250;</button>'
    + '</div>'
    + '<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:0;padding:0 4px ' + (isPC?'2px':'1px') + '">'+hdrHtml+'</div>'
    + '<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:0;padding:0 4px ' + (isPC?'8px':'3px') + '">'+cellsHtml+'</div>'
    + '</div>';
  ...
  if (isPC) {
    ...
    return '<div style="display:flex;flex-direction:column;height:100%;min-height:0;background:#f9fafb">'
      + miniCal
      + '<div id="quick-detail" style="flex:1;overflow-y:auto;-webkit-overflow-scrolling:touch;min-height:0">'+detailHtml+'</div>'
      + '</div>';
  }
}
